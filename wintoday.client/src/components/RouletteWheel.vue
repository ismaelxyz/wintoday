...
watch(
  () => props.spin,
  (val) => {
    if (val && props.spinning) tryStart();
  }
);
...
onMounted(() => {
  if (props.spin && props.spinning) startSpin();
});
...
.wheel {
  ...
  background: #111;
  overflow: visible;
  box-shadow: 0 8px 30px -6px rgba(0, 0, 0, 0.75), inset 0 0 14px #000;
...
.wheel::before {
  ...
  inset: 0;
  border-radius: 50%;
  background: radial-gradient(circle at 50% 50%, #1f2937 0 55%, #000 70% 100%);
  z-index: 0;
}
...
.seg {
  ...
  transform: rotate(var(--rot)) skewY(calc(90deg - 360deg / 37));
  overflow: visible;
  display: flex;
...
.seg::before {
  ...
  position: absolute;
  inset: 0;
  transform: skewY(calc(- (90deg - 360deg / 37))) rotate(calc((360deg / 37) / 2));
  border-radius: 2px;
}
.seg.red::before {
...
.label {
  ...
  top: 50%;
  left: 100%;
  transform: rotate(calc((360deg / 37) / 2)) translate(-90%, -50%) rotate(calc(-1 * var(--rot)));
  font-size: clamp(0.55rem, 0.9vw + 0.25rem, 0.85rem);
  font-weight: 600;
  color: #fff;
...
.hub {
  ...
  justify-content: center;
  align-items: center;
  box-shadow: inset 0 0 6px #000, 0 4px 14px -6px #000;
  z-index: 3;
}
.hub .brand {
  font-size: 1.4rem;
...
.pointer .pin {
  ...
  border-radius: 50%;
  margin-bottom: 4px;
  box-shadow: 0 0 0 3px #92400e, 0 2px 6px -1px rgba(0, 0, 0, 0.7);
}
...
.wheel:after {
  ...
  pointer-events: none;
  box-shadow: 0 0 0 2px rgba(255, 255, 255, 0.05), 0 0 18px -4px rgba(0, 0, 0, 0.9) inset;
}
