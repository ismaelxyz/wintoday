...
const { roundId = "", disabled = false } = defineProps<{
  roundId?: string | null;
  disabled?: boolean;
}>();
...
<template>
  <form class="panel bet-form" @submit.prevent="submit">
    <h3>Hacer Apuesta</h3>
    <div class="field round" v-if="roundId">
      Ronda previa: <code>{{ roundId.slice(0, 8) }}</code>
    </div>
    <label class="field"
      >Monto
      <input
    ...
    <div class="field">
      ...
      <div class="options">
        ...
        <label
          ><input type="radio" value="colorParity" v-model="state.betType" /> Color + Paridad</label
        >
    ...
    <button type="submit" :disabled="!canSubmit || disabled" class="primary w-full">
      🎲 Apostar
    </button>
  </form>
</template>
